# Plot_consistency -  Choline - Todos os Sistemas
...
matplotlib.rcParams[
    'xtick.direction'] = 'in'  # change the ticks direction # mudança da direção dos marcadores ao longo dos eixos de um gráfico que indicam valores específicos
...
font.set_weight(
    'bold')  # 'ultralight', 'light', 'normal', 'regular', 'book', 'medium', 'roman', 'semibold', 'demibold', 'demi', 'bold', 'heavy', 'extra bold', 'black' # peso da fonte
...
opt = {'fillstyle': 'full', 'markersize': 5,
       'markeredgewidth': 0.5}  # aparência dos marcadores, fillstyle: full - estilo de preenchimento markersize: 5 - tamanho do marcador markeredgewidth: 0.5 largura da borda do marcador
...
caminho_arquivo = 'Choline - Todos os Sistemas.xlsx'  # caminho do arquivo
wb = openpyxl.load_workbook(
    caminho_arquivo)  # ler dados de abas individuais do arquivo Excel sem abrir e fechar repetidamente, mantém aberto. Mais eficiente se você precisa ler dados de várias abas do arquivo. Lista com os nomes de todas as abas do arquivo Excel.
...
for i in wb.sheetnames:  # para i dentro da lista wb com os nomes de todas as abas
    ...
    nome_aba_split = nome_aba.title.split(
        "_")  # nome_aba é o nome completo da aba. nome_aba.title converte o nome da aba para o formato de título. split("_ ") divide o nome da aba em duas partes usando o caractere "_ "
    if len(nome_aba_split) > 2:  # se o nome da aba contiver mais de duas partes
        ...
        nomes_abas.append(
            nome_aba.title)  # nome da aba não tiver mais de duas partes, adiciona o nome completo da aba à lista nomes_abas (usada na legenda)
    ...
    for j in nome_aba.iter_rows(min_row=2,
                                values_only=True):  # iter_rows: permite iterar sobre as linhas dessa aba # min_row=2: especifica a primeira linha a ser considerada, nesse caso, começa na segunda linha, pois a primeira linha geralmente contém os cabeçalhos das colunas # values_only=True: indica que queremos obter apenas os valores das células, sem incluir informações adicionais como formatação, fórmulas ou estilos.
        dados_coluna_O.append(j[17])
        dados_coluna_P.append(j[18])
        dados_coluna_Q.append(j[19])
...
symbols = ['o', 's', '^', 'D', 'v', '>', '<', 'P', 'X', 'H', '*', 'h', '+', 'x', '|', '_', '1', '2', '3', '4', 'p', ',',
           '.', '8', 'p', 'd', 'D', '|', '_', '$...$']
...
for i in range(len(dados_coluna_O_array)):
    ...
    symbol = symbols[i]
    color = colors[i]
    line1, = plt.plot(x, y, marker=symbol, markersize=opt['markersize'], markeredgewidth=opt['markeredgewidth'],
                      fillstyle=opt['fillstyle'], color=color, linestyle='none',
                      label=nomes_abas[i])  # lines armazenam as linhas geradas pelos plots
...
plt.legend(handles=lines, loc='upper right', fontsize=4,
           markerscale=0.5)  # cria a legenda usando as linhas armazenadas na lista
...
nome_arquivo_sem_extensao = os.path.splitext(nome_arquivo_python)[
    0]  # obter o nome do arquivo sem extensão # [0] vai acessar o primeiro elemento da tupla retornado por path.splitext
